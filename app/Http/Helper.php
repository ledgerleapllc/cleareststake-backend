...
class Helper {
	...
	public static function getSettings() {
		...
		$totalBalance = User::where('id', '>', 0)
			->where('role', '!=', 'admin')
			->sum('balance');
		$settings['real_total_balance'] = $totalBalance;

		$totalInFundBalance = User::where('id', '>', 0)
			->where('role', '!=', 'admin')
			->where('in_fund', '=', 1)
			->sum('balance');
		$settings['real_total_infund'] = $totalInFundBalance;
	...
	// Add Balance
	public static function addBalance($balance, $inFund) {
	...
	// Subtract Balance
	public static function subtractBalance($balance, $inFund) {
		$settings = self::getSettings();
		$total = isset($settings['total_balance']) ? (int) $settings['total_balance'] : 0;
		$total -= (int) $balance;
	...
	public static function updateUsersBalanceByRate($rate, $sumTarget) {
		...
		if ($users) {
			...
			if ($sumDiff >= 0) {
				...
				if ($count > 0) {
					$each = (int) ($sumDiff / $count);
					$rest = $sumDiff - $each * $count;
